    ...
    <>
          ...
          <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
            <PopoverTrigger asChild>
              <div className="select-none flex items-center gap-2 cursor-pointer hover:bg-natural rounded-lg px-3 py-2 transition-all duration-300">
                {/* User Avatar */}
                <div className="hidden md:block w-10 h-10 rounded-full flex items-center justify-center overflow-hidden transition-all">
                  {userAvatar ? <Image
                    src={userAvatar}
                    alt={userName}
                    width={40}
            ...
            <PopoverContent className="p-2 mr-2 outline-none border-none rounded-xl rounded-t-none">
              <div className="flex md:hidden items-center gap-2 mb-2 p-2 border-b border-gray-100">
                {/* User Avatar */}
      ...
      {isMobileMenuOpen && (
        <>
          {/* Backdrop */}
          <div
            className="fixed inset-0 z-[99] bg-black/50 backdrop-blur-sm xl:hidden"
            onClick={() => setIsMobileMenuOpen(false)}
          />
          <div className="fixed inset-y-0 left-0 z-[100] w-72 bg-dark-blue flex flex-col p-2 xl:hidden animate-in slide-in-from-left duration-300">
            {/* Header */}
            <div className="flex items-start justify-end gap-8 p-4 px-2 mb-4 relative w-full h-fit xl:h-16">
              <Image
                src="/IFriend.svg"
                alt="iFriend Logo"
                width={130}
