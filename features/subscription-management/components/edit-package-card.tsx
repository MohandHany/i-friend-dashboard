...
import { updatePackage, PlanUpdatePayload } from "@/services/queries/subscription/patch/patch-update-package"
...
export function EditPackageCard({ pkg, open, onOpenChange, onUpdated }: EditPackageCardProps) {
  const [name, setName] = React.useState("")
  const [description, setDescription] = React.useState("")
  const [maxChildren, setMaxChildren] = React.useState(1)
  const [isActive, setIsActive] = React.useState(true)
  const [plans, setPlans] = React.useState<PlanUpdatePayload[]>([])
  const [submitting, setSubmitting] = React.useState(false)
  const [errors, setErrors] = React.useState<FormErrors>({})
  const [submitted, setSubmitted] = React.useState(false)

  // Initialize form when pkg changes — only load plans that actually exist
  React.useEffect(() => {
    if (pkg) {
      setName(pkg.name)
      setDescription(pkg.description ?? "")
      setMaxChildren(pkg.maxChildren)
      setIsActive(pkg.isActive)
      setErrors({})
      setSubmitted(false)

      const initialPlans: PlanUpdatePayload[] = pkg.plans.map((existing) => ({
        billingPeriod: existing.billingPeriod,
        price: Number(existing.price),
        currency: existing.currency,
        trialDays: existing.trialDays,
        aiConversationCredits: existing.aiConversationCredits,
        aiReportCredits: existing.aiReportCredits,
  ...
  const updatePlan = (index: number, updates: Partial<PlanUpdatePayload>) => {
  ...
  const validate = (
    ...
    currentPlans: PlanUpdatePayload[]
  ): FormErrors => {
      ...
      <AlertDialogContent className="max-w-4xl p-8 rounded-2xl max-h-[90vh] overflow-y-auto overflow-x-hidden custom-scrollbar">
        ...
        <div className="flex flex-col gap-5">
          ...
          {plans.length === 0 ? (
            ...
            >
              No plans configured. Click &quot;Add Plan&quot; to add one.
            </div>
          ) : (
                    ...
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <div className="flex items-center gap-2">
                        <Switch
                          ...
                          className={cn(
                            "data-[state=checked]:bg-primary-blue h-5 w-9 [&>span]:w-4 [&>span]:h-4 transition-all",
                            !isActive && "invisible"
                          )}
                        />
                        <button
